...
struct NNet *load_network(const char* filename, char* inputFile)
{
    ...
    load_inputs(inputFile, nnet->inputSize, input_prev);
...
void initialize_input_interval(struct NNet* nnet, char *inputFile, int inputSize, float *input, float *u, float *l){
    load_inputs(inputFile, inputSize, input);
    if(PROPERTY == 0){
        for(int i =0;i<inputSize;i++){
            u[i] = input[i]+INF;
            l[i] = input[i]-INF;
...
void load_dave_inputs(int img, int inputSize, float *input){
    if(img>=100000){
...
int evaluate_conv(struct NNet *network, struct Matrix *input, struct Matrix *output){
    ...
    for (layer = 0; layer<(numLayers); layer++)
    {
        ...
        if(nnet->layerTypes[layer]==0){
            ...
            int out_size = 0;
/*
float tmp_out_size =  (padding_size-(kernel_size-1))/stride;
printf("tmp:%f\n",tmp_out_size );
                                     ...
                                     float *wrong_low_s_up, float *wrong_low_s_low)
{
    ...
    for (layer = 0; layer<(numLayers); layer++)
    {
        ...
        for (i=0; i < nnet->layerSizes[layer+1]; i++)
        {
            ...
            if(err_row>0){
                for(int err_ind=0;err_ind<new_equation_err_matrix.row;err_ind++){
                    if(new_equation_err[err_ind+i*new_equation_err_matrix.row]>0){
                        tempVal_upper += new_equation_err[err_ind+i*new_equation_err_matrix.row];
                    }
                    else{
                        tempVal_lower += new_equation_err[err_ind+i*new_equation_err_matrix.row];
                    }
                }
            }
            ...
            if(layer<(numLayers-1)){
                wrong_up_s_up[node_cnt] = tempVal_upper;
                wrong_up_s_low[node_cnt] = tempVal_lower;
                wrong_low_s_up[node_cnt] = tempVal_upper;
                wrong_low_s_low[node_cnt] = tempVal_lower;
                if (tempVal_upper<=0.0){
                    tempVal_upper = 0.0;
                    for(k=0;k<inputSize+1;k++){
                    ...
                    if(new_equation_err_matrix.row>0){
                        for(int err_ind=0;err_ind<err_row;err_ind++){
                            new_equation_err[err_ind+i*new_equation_err_matrix.row] = 0;
                        }
                    }
                    R[layer][i] = 0;
                }
                    ...
                    *wrong_node_length += 1;
                    ...
                    if(err_row>0){
                        ...
                        for(int err_ind=0;err_ind<err_row;err_ind++){
                            new_equation_err[err_ind+i*new_equation_err_matrix.row] *= tempVal_upper / (tempVal_upper - tempVal_lower);
                        }
                    }
                    
                    new_equation_err[*wrong_node_length-1+i*new_equation_err_matrix.row] -= tempVal_upper*tempVal_lower/\
