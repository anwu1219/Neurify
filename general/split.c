...
int check_min_constant(struct NNet *nnet, struct Interval *output){
...
int check_min_constant1(struct NNet *nnet, struct Matrix *output){
                     ...
                     lprec *lp, int *rule_num, int depth)
{
    ...
    sigs2[target] = 0;
    pthread_mutex_lock(&lock);
    if((depth <= avg_depth- MIN_DEPTH_PER_THREAD) && (count<=MAX_THREAD) && !NEED_FOR_ONE_RUN) {
        pthread_mutex_unlock(&lock);
        pthread_t workers1, workers2;
